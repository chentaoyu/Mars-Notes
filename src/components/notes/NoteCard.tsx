...
export function NoteCard({ note, onDelete }: NoteCardProps) {
  ...
  return (
    <Link href={`/editor/${note.id}`}>
      <Card className="note-card cursor-pointer group relative">
        <CardHeader>
          ...
          {note.tags && note.tags.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {note.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag.id}
                  className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300"
